...
/**
 * @brief Setup code, run before anything else
 *
 */
void setup()
{
...
/**
 * @brief Checks to see if there are any leaks in the main chamber
 *
 * @return uint8_t Bit representation of which sensors detect leaks
 */
...
/**
 * @brief Resets connection with topside computer
 *
 */
void reset()
{
...
/**
 * @brief Main event loop, runs after setup is completed
 *
 */
void loop()
{
	...
	if (mode == MODE_COPI)
	{
		// Checks to see if there is a message to read from topside
		if (cipo_driver.readBlocks())
		{
			// Makes sure the message from topside wasn't corrupted
